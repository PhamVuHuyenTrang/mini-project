...
class CUB200(MyCUB200):
    ...
    def enable_return_segmask(self):
        self._return_segmask=True

    def disable_return_segmask(self):
        self._return_segmask=False
    ...
    def __getitem__(self, index: int, ret_segmask=False) -> Tuple[type(Image), int, type(Image)]:
        ...
        if self.transform is not None:
            # check if transform is instance of Compose and get first transform if the first is instance of randomresizecrop
            img = self.transform(img)
...
class SequentialCUB200(ContinualDataset):
    ...
    N_TASKS = 10
    MEAN, STD = (0.4856, 0.4994, 0.4324), (0.2272, 0.2226, 0.2613)
    TRANSFORM = transforms.Compose([  # https://arxiv.org/pdf/2004.10956.pdf, https://openreview.net/pdf?id=tHgJoMfy6nI
